...
4. **收银台：** 预估金额、优惠券抵扣、最终金额。**支持余额支付**（余额不足时先引导用户进行充值；充值通过微信支付完成）。
...
6. **用户余额充值记录表（新增，用于记录每一次通过微信支付完成的余额充值）**

```sql
CREATE TABLE `ums_member_recharge` (
  `id`                  bigint       NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `member_id`          bigint       NOT NULL COMMENT '关联 ums_member.id',
  `recharge_sn`        varchar(64)  NOT NULL UNIQUE COMMENT '充值单号',
  `amount`             decimal(10,2) NOT NULL COMMENT '充值金额(元)',
  `pay_channel`        tinyint      NOT NULL DEFAULT 1 COMMENT '支付渠道：1-微信小程序',
  `pay_status`         tinyint      NOT NULL DEFAULT 0 COMMENT '支付状态：0-待支付，1-支付成功，2-已关闭/失败',
  `wechat_prepay_id`   varchar(128) DEFAULT NULL COMMENT '微信预支付ID',
  `wechat_transaction_id` varchar(128) DEFAULT NULL COMMENT '微信支付订单号',
  `notify_time`        datetime     DEFAULT NULL COMMENT '微信回调时间',
  `create_time`        datetime     DEFAULT CURRENT_TIMESTAMP,
  KEY `idx_member_id` (`member_id`)
) ENGINE=InnoDB COMMENT='用户余额充值记录表';
```

> 业务说明：
>
> - 用户在“钱包充值”页输入充值金额，后端生成一条 `ums_member_recharge` 记录并调用微信支付生成预支付单，将预支付信息返回前端。
> - 微信支付成功异步回调时，根据 `recharge_sn` 或 `wechat_transaction_id` 查找对应记录，在幂等校验通过后：
>   - 将该条记录的 `pay_status` 更新为 `1-支付成功`，写入 `notify_time` 等回调信息；
>   - 将 `amount` 累加到 `ums_member.balance`（用户余额），完成实际充值入账。
> - 用户端的“充值记录”列表接口即基于 `ums_member_recharge` 按 `member_id` 倒序查询。

### 2.2 优惠券相关表结构及变更（新增）

> 说明：优惠券由管理员在后台配置并上架，分为**立减券**与**满减券**两种类型。用户可以通过**余额购买**或**兑换码**领取优惠券，领取后存入用户优惠券表，在创建订单时选择使用并抵扣订单金额。

1. **优惠券模板表（新增，用于定义优惠券规则与上架配置）**

```sql
CREATE TABLE `mkt_coupon` (
  `id`              bigint       NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `name`            varchar(64)  NOT NULL COMMENT '优惠券名称，如 新用户立减10元',
  `coupon_type`     tinyint      NOT NULL COMMENT '优惠券类型：1-立减券(无门槛)，2-满减券(满X减Y)',
  `amount`          decimal(10,2) NOT NULL COMMENT '优惠金额(元)',
  `min_amount`      decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '满减门槛金额(元)，立减券为0',
  `price`           decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '购买价格(余额支付)，0表示不可购买只可兑换',
  `total_count`     int          NOT NULL DEFAULT 0 COMMENT '发放总数量，0表示不限制',
  `receive_limit`   int          NOT NULL DEFAULT 0 COMMENT '每个用户可领取数量上限，0表示不限制',
  `obtain_channel`  tinyint      NOT NULL DEFAULT 0 COMMENT '领取渠道：0-余额购买+兑换码，1-仅余额购买，2-仅兑换码',
...
2. **优惠券兑换码表（新增，用于存储批量生成的兑换码）**

```sql
CREATE TABLE `mkt_coupon_code` (
  `id`          bigint       NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `coupon_id`   bigint       NOT NULL COMMENT '关联 mkt_coupon.id',
  `code`        varchar(64)  NOT NULL UNIQUE COMMENT '唯一兑换码',
  `bound_member_id` bigint   DEFAULT NULL COMMENT '已绑定的用户ID（首次兑换时写入）',
  `used`        tinyint      NOT NULL DEFAULT 0 COMMENT '是否已使用：0-未使用，1-已使用',
  `used_time`   datetime     DEFAULT NULL COMMENT '使用时间',
  `expire_time` datetime     DEFAULT NULL COMMENT '兑换码过期时间（可选，默认跟随优惠券）',
  `create_time` datetime     DEFAULT CURRENT_TIMESTAMP,
  KEY `idx_coupon_id` (`coupon_id`)
) ENGINE=InnoDB COMMENT='优惠券兑换码表';
```

3. **用户优惠券表（新增，用于记录用户实际持有的优惠券）**

```sql
CREATE TABLE `mkt_user_coupon` (
  `id`           bigint       NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `coupon_id`    bigint       NOT NULL COMMENT '关联 mkt_coupon.id',
  `member_id`    bigint       NOT NULL COMMENT '领取人ID，对应 ums_member.id',
  `status`       tinyint      NOT NULL DEFAULT 0 COMMENT '状态：0-未使用，1-已使用，2-已过期',
  `obtain_type`  tinyint      NOT NULL COMMENT '获取方式：1-余额购买，2-兑换码',
  `obtain_time`  datetime     DEFAULT CURRENT_TIMESTAMP COMMENT '领取时间',
  `use_time`     datetime     DEFAULT NULL COMMENT '实际使用时间',
  `order_id`     bigint       DEFAULT NULL COMMENT '使用时关联的订单ID，对应 oms_order.id',
  `expire_time`  datetime     DEFAULT NULL COMMENT '到期时间（按优惠券规则计算）',
...
4. **订单表优惠券字段变更（在原有订单表基础上新增，用于快照记录优惠金额）**

```sql
-- 在 oms_order 上新增与优惠券相关的三个字段
ALTER TABLE `oms_order`
  ADD COLUMN `coupon_id`       bigint       DEFAULT NULL COMMENT '使用的优惠券模板ID，关联 mkt_coupon.id',
  ADD COLUMN `user_coupon_id`  bigint       DEFAULT NULL COMMENT '使用的用户优惠券ID，关联 mkt_user_coupon.id',
  ADD COLUMN `coupon_amount`   decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT '本单优惠金额快照';
```

> 使用规则简要说明：
> - 立减券：不校验 `min_amount`，只要在有效期内并满足其他业务限制即可抵扣 `amount`；
> - 满减券：只有当订单应付金额 ≥ `min_amount` 时才能使用，抵扣金额为 `amount`；
> - 用户下单时前端可传入 `userCouponId`，后端根据 `mkt_user_coupon` 与 `mkt_coupon` 校验合法性与可用性，计算 `coupon_amount` 并更新到 `oms_order.coupon_amount`、`mkt_user_coupon.status/use_time/order_id`。
...
| `/order/pay` | `POST` | 使用余额支付订单 | 校验用户余额是否足够，余额充足则直接扣减 `ums_member.balance` 完成支付；余额不足时返回错误码及“余额不足，请先充值”文案，由前端跳转到钱包充值页。 |
| `/order/list` | `GET` | 个人订单中心 | 支持按状态（待支付、服务中等）过滤。 |
| `/order/cancel` | `POST` | 取消订单 | 判定当前状态，若师傅已出发则涉及违约金计算。 |
| `/wallet/recharge/create` | `POST` | 发起余额充值（微信预下单） | 生成一条充值单（`ums_member_recharge`），调用微信支付 SDK 生成预支付单，将 `prepayId` 返回前端。 |
| `/wallet/recharge/list` | `GET` | 查询余额充值记录 | 根据当前登录用户，分页查询其在 `ums_member_recharge` 中的历史充值记录（金额、时间、支付渠道、状态）。 |
